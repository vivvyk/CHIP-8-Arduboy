...
void emulateCycle(){
  ...
  if(dRAM_1 && dRAM_2)
    opcode = RAM_memory[pcind] << 8 | RAM_memory[(pcind1];
  else if(dRAM_1)
    opcode = RAM_memory[pc % I] << 8 | memory[pc+1];
  else if(dRAM_2)
    opcode = memory[pc] << 8 | RAM_memory[(pc+1) % I];
  else{
    ...
    for (int row = 0; row < heightl; row++){
      for(int col = 0; col < 8; col++){
        ...
        if(pixel & (0x80 >> col) != 0){
          ...
          if(arduboy.getPixel(pos1, pos2) == 0xFF){ //TEST EQUIVALENCE FIRST!
