    ...
    for(int row = 0; row < heightl; row++){
     ...
     for(int i = 0; i < 16; i++){
      ...
      for(int col = 0; col < 8; col++){    
        ...
        if(px != 0){
          int pos1 = 2 * ((V[x] + col) % 64);
          int pos2 = 2 * ((V[y] + row) % 32);

          //V[y] not updated
          int pixState = arduboy.getPixel(pos1, pos2);
          if(pixState != 0){
            arduboy.drawPixel(pos1, pos2, BLACK);
            arduboy.drawPixel(pos1 + 1, pos2, BLACK);
            arduboy.drawPixel(pos1, pos2 + 1, BLACK);
            arduboy.drawPixel(pos1 + 1, pos2 + 1, BLACK);
            V[15] = 1; 
          }else{
            arduboy.drawPixel(pos1, pos2, WHITE);
            arduboy.drawPixel(pos1 + 1, pos2, WHITE);
            arduboy.drawPixel(pos1, pos2 + 1, WHITE);
            arduboy.drawPixel(pos1 + 1, pos2 + 1, WHITE);
         }
        }
    ...
    if(last_digits == 0x07){
      ...
      for(int i = 0; i < 16; i++){
        if(i == x + 1)
          break;
        //Final memory read.
        if(I + i == RAM_indices[(I + i) % 16]){
          V[i] = RAM_memory[(I + i) % 16];
        }else{
